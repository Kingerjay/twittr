...
export const PostDetail = ({postId}: Props) => {
   ...
   // Fetch Post Details
   const {data, error, isLoading: postLoading} = useQuery<Post, Error>({
   ...
   const { data: commentCount, isLoading: commentLoading } = useQuery<number, Error>({
   queryKey: ["commentCount", postId],
   queryFn: () => fetchCommentCount(postId),
   refetchInterval: 1000, // Refresh comment count every 5 seconds
 });
  ...
  return (
    <div className="w-full h-screen text-[#E7E9EA] flex flex-col overflow-y-auto scrollbar-hide">
         ...
         <div className="px-2 sm:px-4 flex justify-between items-center sticky top-0 bg-black/80 backdrop-blur-md z-10 mb-2 sm:mb-4">
           ...
           <Link to="/"><ArrowLeft size={18} className="sm:w-[20px] sm:h-[20px]" /></Link>
           </button>
           ...
           <div className="flex py-2 gap-2 sm:gap-4 items-center">
             <button className="flex items-center justify-center rounded-4xl bg-black py-0.5 px-2 sm:px-4 border border-[rgb(84,90,106)] font-semibold text-base sm:text-lg">Reply</button>
         ...
         <div className="flex items-center justify-between">
           ...
           {data.avatar_url ? (
             ...
             <div className="w-[35px] h-[35px] sm:w-[45px] sm:h-[45px] rounded-full bg-gradient-to-tl from-[#8A2BE2] to-[#491F70] text-xl sm:text-2xl flex items-center justify-center font-bold capitalize" >{data.username.slice(0,1)}</div>
           )}
           <div className="flex flex-col flex-1">
             <div className="leading-[20px] sm:leading-[22px] font-semibold text-sm sm:text-base">
               {data.full_name} <p className="text-gray-400 text-xs sm:text-sm">@{data.username}</p>
             </div>         
           </div>
         </div>
         ...
         <div className="mt-1 sm:mt-2 flex-1">
           ...
           <div className="my-2 sm:my-4 text-sm sm:text-[17px] break-words">
             {data.content}
           </div>
         ...
         <div className="mt-2 sm:mt-4 px-2 sm:px-4 py-1 sm:py-1.5 mx-2 sm:mx-4 border-y border-[rgb(84,90,106)] flex items-center justify-between sm:justify-start sm:gap-16 md:gap-25">
           ...
           <span className="ml-0.5 text-sm sm:text-[16px]">{commentLoading ? "..." : commentCount}</span>
           </div>
